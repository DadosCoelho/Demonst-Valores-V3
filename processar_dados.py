...
def extract_formula_from_description(description):
    ...
    if isinstance(description, str):
        ...
        if match:
            ...
            # Permite números, ., espaços, +, -, *, /, parênteses para aninhamento
    ...
    for account in accounts_list:
        ...
        if parent_code:
            ...
            if parent_code in account_dict:
                 ...
                 # Tratar caso de conta órfã cujo pai não está na lista (pode ser um erro na planilha)
                 # Neste caso, a conta será tratada como de nível superior para fins de cálculo hierárquico
                 pass # Não adiciona ao children_map, o pai implícito não existe no conjunto
...
def get_calculated_value(account_code, period, account_dict, children_map, raw_data_for_ds_dict, ds_name):
    ...
    if account["tipo"] == "analitica":
            ...
            for raw_code in linked_raw_codes:
                ...
                if raw_data_entry and raw_data_entry.get("valores"):
                    ...
                    total_analitica_value += numeric_raw_value
                     ...
                     # Aviso se um dos códigos brutos vinculados não foi encontrado no DataSource para o período específico
                     # print(f"Aviso: Código de dado bruto '{raw_code}' (vinculado à conta '{account_code}') não encontrado no DataSource '{ds_name}' para o período '{period}'.")
                     pass # Simplesmente não adiciona à soma se o dado bruto não for encontrado ou não tiver valor para o período
    ...
    elif account["tipo"] == "calculo" and account["formula"]:
        # Para contas de cálculo, avaliamos a fórmula
        ...
        try:
            ...
            for ref_code_tuple in referenced_codes:
                ...
                # Usa um prefixo seguro para evitar conflitos com nomes de funções built-in se usar eval
            ...
            # Adicionar suporte básico para SUM() se suas fórmulas usarem isso
            # Esta parte precisaria de lógica para pré-processar a fórmula e converter SUM(a,b) para (a+b) ou similar.
            # A regex atual só captura códigos numéricos. Se sua fórmula usa nomes (ex: "Receita - Custo"),
            # você precisaria estender a regex e a lógica de substituição.
            # Ex: eval("v_001 + v_004") com local_vars = {'v_001': valor_de_001, 'v_004': valor_de_004} funcionará.
            # Se for "SUM(v_001, v_004)", eval vai falhar a menos que você defina uma função SUM no escopo local.

            # Avalia a expressão usando eval(). CUIDADO! `eval` é perigoso com inputs não confiáveis.
            # Assumimos que as fórmulas na planilha são controladas e seguras.
            # Se a fórmula for complexa ou puder conter código malicioso, NÃO use eval.
            # Use um ambiente global vazio ({}) e o ambiente local (`local_vars`) para isolamento.
            ...
            # Imprime um erro mais detalhado para debug
...
def processar_planilha_integrado(caminho_excel='DADOS.xlsx'):
    ...
    for sheet_name in xls.sheet_names:
         ...
         try:
             ...
             # Verifica se a primeira célula da primeira coluna é igual ao nome da coluna (indicativo de cabeçalho repetido)
             if not df_dados.empty and str(df_dados.iloc[0][coluna_codigo_raw]).strip().lower() == str(coluna_codigo_raw).strip().lower():
                  ...
                  # Re-identificar colunas caso tenham sido afetadas pela remoção de linha
                  # Esta parte pode precisar de ajuste se houver muitas linhas de cabeçalho
                  colunas_dados_raw = df_dados.columns.tolist()
                  coluna_codigo_raw = colunas_dados_raw[0]
                  coluna_descricao_raw = colunas_dados_raw[1]
                  colunas_periodos_raw = colunas_dados_raw[2:]
             ...
             for periodo_header_raw in colunas_periodos_raw:
                 if isinstance(periodo_header_raw, pd.Timestamp):
                     ...
                     formatted_header = periodo_header_raw.strftime('%b/%Y')
                 else:
    ...
    for sheet_name in xls.sheet_names:
         ...
         try:
             ...
             linked_data_sources_in_plan = set() # Rastreia quais DataSources este plano *referencia* nas colunas de vínculo (existam eles ou não nas abas de dados)
             ...
             for col_header_raw in colunas_vinculo_raw:
                  ...
                  if ds_name_from_col:
                       ...
                       # print(f" - Coluna '{col_header_raw}' mapeada para data source '{ds_name_from_col}'.") # Muito verboso
             ...
             for index, row in df_plano.iterrows():
                 ...
                 tipo = str(row[coluna_tipo_raw]).strip().lower() if pd.notnull(row[coluna_tipo_raw]) else None

                 if not codigo: # Pula linhas sem código na coluna de código
                      continue

                 formula = None
                 if tipo == 'calculo' and descricao:
                     formula = extract_formula_from_description(descricao)
                     # Opcional: Remover a fórmula da descrição exibida?
                     # if formula:
                 ...
                 accounts_list.append({
                     ...
                     "data_sources": data_sources_map, # Mapeamento {NomeDataSource: "CodigoBruto1; CodigoBrigo2"}
                     "valores": {} # Inicializa o dicionário de valores calculados/obtidos para CADA DataSource (será preenchido depois)
                 })
             ...
             if accounts_list:
                 ...
                 all_plans[plan_name] = {
                     ...
                     # Filtra linked_data_sources para incluir apenas aqueles que foram lidos com sucesso nas abas 'dados'
    ...
    global calculation_cache # Usa o cache global (global para o script, limpo por visão)
    ...
    for plan_name, plan_info in all_plans.items():
        ...
        linked_data_sources = plan_info["linked_data_sources"] # Já filtrado para DSs encontrados
        ...
        for ds_name in sorted(linked_data_sources): # Itera apenas sobre os DSs encontrados
            # print(f" - Gerando visão calculada para '{plan_name}' usando DataSource '{ds_name}'...") # Muito verboso

            raw_data_info = all_raw_data[ds_name] # Pega os dados pré-carregados deste DataSource
            raw_data_for_ds_dict = raw_data_info["data"] # Dicionário {codigo: {...}} para lookup rápido
            periodos_ds = raw_data_info["periodos"] # Lista ordenada de períodos
            ...
            # Isso garante que os cálculos de cada visão (Plano + DataSource) sejam independentes e usem apenas os dados do DS atual.
            calculation_cache = {}
            ...
            for account in accounts_list:
                 ...
                 # Remove campos que não são necessários na saída final da visão (ou mantém para debug se preferir)
                 if "data_sources" in calculated_account:
                      del calculated_account["data_sources"] # Remove o mapeamento bruto para os DataSources

                 if calculated_account.get("formula") is None:
                      if "formula" in calculated_account: # Verifica se a chave existe antes de tentar deletar
                           del calculated_account["formula"] # Remove se não tiver fórmula definida
...
def update_index_html_with_json(caminho_html='index.html', dados_json={}):
    ...
    """
    # Define o nome da variável JavaScript onde os dados serão armazenados no HTML.
    # O script JS no index.html deve usar este mesmo nome (calculatedViewsData).
    js_variable_name = 'calculatedViewsData'

    # Define o padrão regex para encontrar a definição da variável alvo no HTML.
    # Procura por `const nomeDaVariavel = ...;` ou `let nomeDaVariavel = ...;` ou `var nomeDaVariavel = ...;`
    # Captura 1: `const ` ou `let ` ou `var ` seguido do nome da variável e `= ` (Ex: `const calculatedViewsData = `)
    # Captura 2: o conteúdo que está entre `=` e `;` (o JSON existente ou placeholder)
    # Captura 3: `;`
    # Usa re.DOTALL para que '.' case com quebras de linha, útil se o JSON for multi-linha.
    # Usa raw string (r'') e escapa barras invertidas (\) e parênteses (\( \)) para o regex.
    # O '|' dentro do regex cria um grupo de alternância para 'const|let|var'.
    pattern = r'(const|let|var)\s+' + re.escape(js_variable_name) + r'\s*=\s*(\{.*?\});'

    html_content = None
    try:
        # Tenta ler o conteúdo do arquivo index.html existente com encoding UTF-8.
        with open(caminho_html, 'r', encoding='utf-8') as f:
            html_content = f.read()
        print(f"Arquivo '{caminho_html}' encontrado. Lendo conteúdo existente.")

    except FileNotFoundError:
        # Se o arquivo index.html não for encontrado, informa o usuário.
        ...
        return False # Indica falha, pois não há arquivo para editar.

    # Formata o JSON a ser injetado.
    # indent=4 para legibilidade no arquivo HTML.
    # ensure_ascii=False para manter caracteres especiais (acentos, etc.).
    # sort_keys=False para tentar preservar a ordem original das contas/períodos.
    # separators para formatação JSON padrão.
    json_data_str = json.dumps(dados_json, indent=4, ensure_ascii=False, sort_keys=False, separators=(',', ': '))

    # Conteúdo completo da definição da variável JS a ser injetada/substituída.
    full_json_definition_line = f"const {js_variable_name} = {json_data_str};"

    # Tenta encontrar o padrão `const calculatedViewsData = ...;` (ou let/var) no conteúdo HTML lido.
    match = re.search(pattern, html_content, re.DOTALL)

    if match:
        # Se o padrão for encontrado, substitui APENAS o conteúdo entre `=` e `;` (Grupo 2)
        # pelo novo JSON formatado.
        print(f"Padrão '{match.group(1)} {js_variable_name} = ...;' encontrado no arquivo '{caminho_html}'. Substituindo dados...")
        # Reconstrói o conteúdo HTML: parte antes do JSON + novo JSON + parte depois do JSON.
        # Usa start(2) e end(2) para pegar o span exato do conteúdo do JSON placeholder.
        novo_html_content = html_content[:match.start(2)] + json_data_str + html_content[match.end(2):]

    else:
        # Se o padrão NÃO for encontrado no arquivo existente, tenta injetar a definição completa da variável.
        ...
        # Busca pela primeira tag de fechamento de script (</script>).
        # Usa raw string e escapa a barra (\).
        script_end_tag_pattern = r'</script>'
        match_script_end = re.search(script_end_tag_pattern, html_content, re.DOTALL)

        if match_script_end:
             # Se encontrar </script>, injeta a definição COMPLETA da variável JSON antes dela, dentro do bloco script.
             # Adiciona quebras de linha e indentação básica para legibilidade no código fonte do HTML.
             # A string injetada será: `// Dados injetados pelo script Python:\n const calculatedViewsData = { ... };\n\n`
             injection_string = f"\n\n        // Dados injetados pelo script Python:\n        {full_json_definition_line}\n\n        " # Note a indentação (espaços) para dentro do script tag
             # Insere a string de injeção antes do início da tag </script> encontrada.
             ...
             # Se nem o padrão nem a tag </script> forem encontrados (situação incomum para um HTML válido),
             # não é possível injetar os dados de forma segura.
             ...
             return False # Indica falha no processo de atualização


    # Se chegamos até aqui, novo_html_content foi criado (seja por substituição ou injeção).
    try:
        # Escreve o conteúdo HTML modificado de volta no arquivo index.html.
        # Usa codificação UTF-8 para garantir que caracteres especiais sejam salvos corretamente.
        with open(caminho_html, 'w', encoding='utf-8') as f:
            f.write(novo_html_content) # Escreve o conteúdo modificado
        print(f"Arquivo '{caminho_html}' atualizado com sucesso!")
        return True # Indica sucesso no processo de atualização
    except Exception as e:
        # Captura e imprime erros durante a escrita do arquivo.
        print(f"Erro ao escrever no arquivo '{caminho_html}': {e}")
        return False # Indica falha na escrita do arquivo


# --- Bloco de Execução Principal ---
# Este código só será executado quando o script for rodado diretamente (não importado como módulo).
if __name__ == "__main__":
    # Obtém a data e hora atual para incluir na mensagem de conclusão.
    ...
    nome_arquivo_excel = 'DADOS.xlsx' # Define o nome esperado do arquivo Excel de entrada.
    # Define o nome do arquivo HTML a ser editado. Assumimos que é 'index.html'.
    nome_arquivo_html = 'index.html' # <--- Altere aqui para 'index.html'


    print(f"Iniciando processamento integrado do arquivo '{nome_arquivo_excel}'...")

    # Verifica se o arquivo Excel existe no diretório atual antes de tentar processá-lo.
    if not os.path.exists(nome_arquivo_excel):
        ...
        # Se o arquivo existe, chama a função principal para processar a planilha e obter os dados extraídos/calculados.
        dados_extraidos = processar_planilha_integrado(nome_arquivo_excel)

        # Verifica se o processamento retornou dados (não None, indicando falha na leitura inicial)
        if dados_extraidos is not None:
             # Verifica se alguma visão calculada foi gerada
            if dados_extraidos.get("calculated_views"):
                # Adiciona timestamps ao dicionário de dados extraídos para registro no JSON de saída.
                ...
                # Tenta atualizar o arquivo index.html com os dados extraídos/calculados.
                if update_index_html_with_json(nome_arquivo_html, dados_extraidos):
                    ...
                    # Informação adicional para o usuário sobre as visões geradas.
                    ...
                    print("2. Verifique as fórmulas das contas do tipo 'calculo' na planilha (entre parênteses na descrição). A avaliação de fórmula implementada é básica (suporta +, -, *, /, parênteses e códigos de contas como referências numéricas) e pode não suportar fórmulas complexas ou com nomes de texto. Erros na fórmula resultarão em valor 0.0.")
                    ...
                    print("4. Verifique se as abas de dados e plano têm as colunas obrigatórias na ordem correta (Código, Descrição, Tipo para plano; Código, Descrição, Períodos... para dados).")
                     ...
                     # Mensagem se houve falha no processo de atualização do arquivo HTML.
                     print(f"\nProcessamento concluído, mas houve um erro ao atualizar o arquivo '{nome_arquivo_html}'.")


            else:
                 # Mensagem se nenhum plano ou DataSource foi lido/vinculado corretamente para gerar visões.
             ...
             # Mensagem se houve falha total na leitura inicial do arquivo Excel.
